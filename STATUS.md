...
| 8 | **Data half DONE 2026-08-27** — Oskaras ran the pipeline locally: **`DATASET-998919e0db23`** = 90 files (30 pairs × 1d/4h/1h since 2021), **1,792,725 bars, zero gaps, zero duplicates**; post-2021 listings correctly start at their listing dates. Committed to `verslas-signals/data/`. **Remaining:** run the WP1-review checks locally — `python -m engine.data.audit data/raw` (UTC alignment, 1h→4h/1d aggregation test, zero-volume/spike/volume-semantics QA) and `python -m engine.universe data/raw/okx/1d --out data/universe.csv` (point-in-time universe + always-in subset), then commit `data/universe.csv` and paste the audit output; verify costs.yaml VERIFY values (Nojus); lock holdout dates at the build session | Oskaras | 9 Sep |
