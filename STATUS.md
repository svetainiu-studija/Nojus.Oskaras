...
| 8 | **Data half DONE 2026-08-27** — Oskaras ran the pipeline locally: **`DATASET-998919e0db23`** = 90 files (30 pairs × 1d/4h/1h since 2021), **1,792,725 bars, zero gaps, zero duplicates**; post-2021 listings correctly start at their listing dates. Committed to `verslas-signals/data/`. **Audit + universe run 2026-08-28** — results: alignment clean (daily bars = UTC midnight); 2022-12-18 exchange-wide outage identified (zero-volume bars, all pairs); >40% daily moves reviewed = real events; native-1d volume disagrees with 1h sums on ~5% of days → **D-019: canonical 4h/1d derived from 1h**. **Remaining:** `git pull`, rerun `python -m engine.data.audit data/raw` (expect PASS), run `make derive` (or `python -m engine.data.derive data/raw/okx/1h data/derived/okx` then the manifest command it prints), commit `data/universe.csv` + the derived DATASET manifest; verify costs.yaml VERIFY values (Nojus); lock holdout dates at the build session | Oskaras | 9 Sep |
