...
class Hyp001:
    ...
    def _pullback_ok(self, pair, d, i):
        ind = self.ind[pair]
        ...
        for k in range(i - run + 1, i + 1):
            ...
            if vs is None or vol[k] >= vs:
                return False
        r = ind["rsi"][i]
        return r is not None and PARAMS["rsi_lo"] <= r <= PARAMS["rsi_hi"]
    ...
    def on_close(self, sim, pair, i, ts):
        ...
        if pair not in self.armed and self._trend_ok(pair, d, i) \
                and self._pullback_ok(pair, d, i):
            self.armed[pair] = i
        return None
